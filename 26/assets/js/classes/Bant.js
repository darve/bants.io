...
var Bant = Class.extend({
	...
	init: function( position, vector ){
		...
		this.speed = 2;
		// this.rotation = FUNK.canvas.width > 420 ? .8 : 2;
		this.vector.rotate(Math.random()*32-Math.random()*32);
	...
	calculate: function() {
		...
		// if ( (typeof FUNK.vector != 'undefined') && (this.pos.isCloseTo( FUNK.vector, 1024 ) ) ) {
		...
		// } else {
			...
			// this.vector.rotate( this.rotation*FUNK.vector.y/99 );	
	...
	draw: function() {
		...
		// FUNK.ctx.lineTo(0,0);
      	...
      	FUNK.ctx.arc(0,0, 3, 0, 2 * Math.PI, false);
