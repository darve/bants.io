...
var FUNK = {
	...
	init: function(){
		...
		if ( window.innerWidth <= 320 ) {
			console.log('MOBILE');
			$('canvas')[0].width = 640;
			$('canvas')[0].height = 1172;
		} else {
			FUNK.canvas.width = 768;
			FUNK.canvas.height = 1024;
		...
		$(document).on('mousemove', 'canvas', function(event){
			if ( FUNK.mousedown === true ) {
				var rect = FUNK.canvas.getBoundingClientRect();
				// FUNK.daveX = event.pageX - rect.left;
				// FUNK.daveY = event.pageY - rect.top;
				FUNK.daveX = event.offsetX;
				FUNK.daveY = event.offsetY;
				FUNK.vector = new Vector2( FUNK.daveX, FUNK.daveY );
			}
		});
		...
		FUNK.ctx.fillStyle = "rgba(255,255,255,1)";
	...
	animate: function(){
		...
		// if ( FUNK.bants[0] ) console.log( FUNK.bants[0].pos.y / FUNK.canvas.height );
		...
		if ( FUNK.mousedown && FUNK.framenum % 6 === 0 && FUNK.vector ) {
		...
		// if ( FUNK.bants[0] ) console.log( FUNK.bants[0].pos.y );
		...
		if ( FUNK.bants.length > 2 ) { 
			...
			FUNK.ctx.strokeStyle = 'rgba(255,255,255,.03)';
