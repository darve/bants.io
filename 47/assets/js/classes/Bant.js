...
var Bant = Class.extend({
	...
	init: function( position, velocity ){
		...
		this.velocity = new Vector2( Math.random() >= .5 ? Math.floor( Math.random() * -1) : Math.floor(Math.random() * 1.5) , Math.random() >= .5 ? Math.floor( Math.random() * -1) : Math.floor(Math.random() * 1.5) ).divideEq(4);
