...
var FUNK = {
	...
	animate: function(){
		...
		// if ( FUNK.framenum % 9 === 0 ) {
			// if ( FUNK.bants.length < $(window).innerWidth() + 100 ) {
			// if ( FUNK.bants.length < 50 ) {
			if ( FUNK.mousedown === true ) {
				FUNK.bants.push( new Bant( new Vector2( FUNK.canvas.width/2, FUNK.canvas.height/2), 'random'));
				// FUNK.bants.push( new Bant( new Vector2( FUNK.vector.x, FUNK.vector.y), new Vector2( 0, -1) ));
			}
		// }
