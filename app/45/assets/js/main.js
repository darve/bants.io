...
var FUNK = {
	...
	animate: function(){
		...
		for ( var i = 0; i < FUNK.bants.length; i++ ) {
			...
			points2.push( FUNK.bants[i].pos.x + ( Math.random() * 12 ));
			points2.push( FUNK.bants[i].pos.y + ( Math.random() * 12 ));

			points3.push( FUNK.bants[i].pos.x + ( Math.random() * 12 )-12);
			points3.push( FUNK.bants[i].pos.y + ( Math.random() * 12 )-12);
		};
